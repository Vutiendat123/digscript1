...
-- Tabs
local MainTab = Window:AddTab("Main", "shovel")
local TeleportTab = Window:AddTab("Teleport", "door-closed")
local MovementTab = Window:AddTab("Movement", "person-standing")

-- Main Tab
local DigSection = MainTab:AddSection(" Dig")

local AutoDigToggle = MainTab:AddToggle({
   Name = "Auto Dig",
   CurrentValue = false,
   Flag = "AutoDig",
...
local SellSection = MainTab:AddSection(" Sell")

local SellAllButton = MainTab:AddButton({
   Name = "Sell All Items",
   Callback = function()
        task.spawn(function()
            local args = {
...
local AutoSellSlider = MainTab:AddSlider({
...
local JournalSection = MainTab:AddSection(" Journal")

local ClaimJournalsButton = MainTab:AddButton({
...
local CharmsSection = MainTab:AddSection(" Charms")
...
local CharmDropdown = MainTab:AddDropdown({
   Name = "Buy Charms",
...
local BuyAgainButton = MainTab:AddButton({
   Name = "Buy Again",
   Callback = function()
        if selectedCharm then
            buyCharm(selectedCharm)
...
local QuestSection = MainTab:AddSection(" Quests")

local AutoPizzaToggle = MainTab:AddToggle({
...
local EnchantmentButton = TeleportTab:AddButton({
...
local MeteorButton = TeleportTab:AddButton({
...
local MerchantButton = TeleportTab:AddButton({
...
local SpawnDropdown = TeleportTab:AddDropdown({
...
local PurchasableDropdown = TeleportTab:AddDropdown({
...
local BossDropdown = TeleportTab:AddDropdown({
...
local BossHitToggle = TeleportTab:AddToggle({
   Name = "Boss Hit",
   CurrentValue = false,
   Flag = "BossHit",
   Callback = function(Value)
...
local GravitySlider = MovementTab:AddSlider({
   Name = "Gravity",
   Range = {0, 999},
   Increment = 1,
   Suffix = "Gravity",
...
local FOVSlider = MovementTab:AddSlider({
   Name = "FOV",
   Range = {20, 120},
   Increment = 1,
   Suffix = "FOV",
...
local InfJumpToggle = MovementTab:AddToggle({
   Name = "Infinite Jump",
   CurrentValue = false,
   Flag = "InfJump",
   Callback = function(Value)
...
local ResetButton = MovementTab:AddButton({
   Name = "Reset Player",
   Callback = function()
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
...
-- Player Teleport
local PlayerSection = MovementTab:AddSection("Player Teleport")
...
local PlayerDropdown = MovementTab:AddDropdown({
...
local RefreshPlayersButton = MovementTab:AddButton({
